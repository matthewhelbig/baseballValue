...
```{r, echo = FALSE}
head(Batting_Value_Salary_2[,1:7])
```
...
```{r}
Batting_Value_Salary <- subset(Batting_Value_Salary_2, Batting_Value_Salary_2$PA >= Batting_Value_Salary_2$AB)
```
...
```{r}
temp_top_excess_value <- age_salary_subset[ age_salary_subset$excess_value_plus >= age_salary_subset$excess_value_plus[order(age_salary_subset$excess_value_plus, decreasing=TRUE)][50] , ]
top_excess_value <- temp_top_excess_value[order(-temp_top_excess_value$excess_value_plus),]
head(top_excess_value[,1:7])
```

While we identified fifty total players who provided considerable excess value, for the purpose of our model we'll be picking five. For various reasons, a majority of the players that were identified via the excess value plus leaderboards did not fit our criteria. In some instances, it was star players who simply had a rare bad season the year before. In other instances, it was a veteran who was signing for a cheap deal at the end of his career.
...
```{r, echo=FALSE}
pre_breakout <- Batting_Value_Salary[ which(Batting_Value_Salary$AutoNum == 4691 | 
                             Batting_Value_Salary$AutoNum == 4441 |
                             Batting_Value_Salary$AutoNum == 4542 |
                             Batting_Value_Salary$AutoNum == 4602 |
                             Batting_Value_Salary$AutoNum == 4601
                             ),]
```
...
Finally, we'll create a subset that just has our "Total" values. This will help us with working on our difference score calculations.
```{r}
...
In order to identify which players are best represented by our determinative statistics, we can develop a difference score formula. A player with a score closer to 0 will be closer to the average for our chosen statistics and would theoretically be a player identified by the model as an undervalued asset. It looks as follows:
```{r}
...
temp_DiffSc2 <- ((total_sub$True_BB_plus - Batting_Value_Salary$BB_plus)/total_sub$True_BB_plus) +
  ...
  ((total_sub$slugging_percentage - Batting_Value_Salary$slugging_percentage)/total_sub$slugging_percentage)
...
head(pre_breakout_two[,1:7])
```

```{r, echo = FALSE}
mean(pre_breakout_two$DiffSc)
```
These selected players have an average difference score of 5, which indicates that their statistics are very similar to the players upon whom we built the model.

**Selected stats from season pre-breakout**
```{r}
test3 <- select(pre_breakout_two, name_common, salary, excess_value, excess_value_plus, DiffSc)
View(test3)
```
...
On 11/3, we'll want to do the following things:

1. 1. Display appropriate pre-breakout and post-breakout stats for the players we chose. Maybe append the total to the bottom like we did starting on line 688. Then compare the pre-breakout totals to the post-breakout totals and show how much value our model extracted (could be good to show the same pre-breakout and post-breakout stuff for the players that comprise our model too)
