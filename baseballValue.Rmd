...
```{r}
pre_K_percentage <- ((Batting_Value_Salary$SO)/(Batting_Value_Salary$AB + Batting_Value_Salary$BB + Batting_Value_Salary$HBP + Batting_Value_Salary$SF)*100)
...
Tomorrow, 4/17, we'll start by subsetting our data to include only players who are 27 years old and older. Then, we'll limit ourselves to players making under $10 million, since spending a ton of money on these guys defeats the purpose. Based on that, we can take the Top however many players (5, 10, 25) and find their statistics in the year before their "breakout" seasons. Then we can aggregate those "seasons before" into one pre-breakout player. (As an aside, it may be good to pick and choose which players we use, for example maybe not Paul Goldschmidt since he was a top prospect, and maybe not the second or third Donaldson or Zobrist seasons). 
...
After that, we can get 2017 and 2018 data (maybe from the Lahman database again) and see which of our poised players actually broke out. Then we can discuss why/why not.
