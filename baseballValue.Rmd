...
So all the data we need is loaded into Value_Salary. We need to demonstrate in this document how we got it from the duplicate stage to the working stage. The "Create year...," "Eliminate...," "Bind...," are what we want to move over to the R Markdown document.

Then we'll have all our batting data read in! The next thing will be to read in all of our pitcher data. The good news is that they say 80% of a project is cleaning data, so we're basically 40% done with our project now. We can follow a lot of the same steps for reading in the pitcher data now that we have those steps listed above.
