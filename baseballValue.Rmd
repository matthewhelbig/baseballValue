...
Today we created some new variables which we'll use to do our major analyses. Tomorrow we'll work on showing that our data needs to be subsetted by age, explain why that is (and what causes that problem (low salaries for young players)), and then subset it as appropriate. It'll also be a good time to do our first plot, maybe of excess value plus against age? The first thing we'll want to do are the steps laid out in baseballValue.R and then proceed from there. Eventually it will be good to regress our dependent variables on "Excess_value_plus" or maybe "Value_plus" since those are what we want to measure eventually.
